...
export const config = {
  ...
  systemPrompt: [
    ...
    "",
    "## Testing — prefer real I/O over mocks",
    "",
    ...
    "(e.g. `.omega/test-sessions/` instead of `.omega/sessions/`), not by mocking I/O away",
    "or deleting output after each test. Use a unique name per test run (timestamp + counter",
    "or random suffix) so tests can run in parallel without conflicts. Let test artifacts",
    "accumulate — they become inspectable evidence and catch regressions in read/replay paths.",
    "Mock external services (LLMs, third-party APIs) because they are slow, unreliable, and",
    "costly — but use real I/O for your own storage. See `manifest.md` § 'Testing philosophy'",
    "for the full rationale and the Omega-specific conventions.",
  ].join("\n"),
};
