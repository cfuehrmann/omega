          ...
          <table class="token-legend-table">
            <tbody>
              <Show when={!isOpenAi()} fallback={<>
                <tr><td><code>new</code></td><td>Total prompt tokens sent</td></tr>
                <tr><td><code>out</code></td><td>Tokens generated by the model</td></tr>
              </>}>
                <tr><td><code>new</code></td><td>Fresh input tokens (full price)</td></tr>
                <tr><td><code>write</code></td><td>Tokens written into prompt cache (1.25×)</td></tr>
                <tr><td><code>read</code></td><td>Tokens served from prompt cache (0.1×)</td></tr>
                <tr><td><code>out</code></td><td>Tokens generated by the model</td></tr>
              </Show>
            </tbody>
          </table>
  ...
  switch (e.type) {
    ...
    case "turn_end": {
      ...
      const cacheDetail = (m.cacheCreationTokens || m.cacheReadTokens)
        ? `  write: ${m.cacheCreationTokens ?? 0}  read: ${m.cacheReadTokens ?? 0}`
        : "";
      const line = `new: ${m.inputTokens}${cacheDetail}  out: ${m.outputTokens}  model: ${e.model}`;
      return (
...
function StickyMetricsBar() {
  ...
  const buildCells = (m: StickyMetrics): Array<[string, string, boolean]> => {
    ...
    if (isOpenAi()) {
      cells.push(["new", String(m.freshInTokens), false]);
    } else {
      cells.push(["new",   String(m.freshInTokens),  false]);
      cells.push(["write", String(m.writeInTokens),  false]);
      cells.push(["read",  String(m.readInTokens),   false]);
    }
