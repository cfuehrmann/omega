...
describe("computeDurations — LLM time", () => {
  ...
  it("sums multiple llm call durations across a tool loop", () => {
    const events: ServerMessage[] = [
      ...
      { type: "tool_result", time: ms(BASE, 800), id: "t1", name: "read_file", isError: false, durationMs: 300, output: "x"},
...
describe("computeDurations — tool time", () => {
  it("measures a single tool batch (one tool)", () => {
    const events: ServerMessage[] = [
      ...
      { type: "tool_result", time: ms(BASE, 900), id: "t1", name: "run_command", isError: false, durationMs: 400, output: "ok"},
  ...
  it("measures a parallel batch: span = max(result.time) − min(call.time)", () => {
    const events: ServerMessage[] = [
      ...
      { type: "tool_result", time: ms(BASE, 600), id: "t1", name: "run_command", isError: false, durationMs: 400, output: "a"},
      { type: "tool_result", time: ms(BASE, 800), id: "t2", name: "read_file", isError: false, durationMs: 590, output: "b"},
  ...
  it("sums tool spans across multiple batches", () => {
    const events: ServerMessage[] = [
      ...
      { type: "tool_result", time: ms(BASE, 400), id: "t1", name: "read_file", isError: false, durationMs: 300, output: "x"},
      ...
      { type: "tool_result", time: ms(BASE, 900), id: "t2", name: "write_file", isError: false, durationMs: 200, output: "ok"},
...
describe("computeLiveDurations — tool time", () => {
  it("flushes a batch as soon as all results arrive", () => {
    const events: ServerMessage[] = [
      ...
      { type: "tool_result", time: ms(BASE, 500), id: "t1", name: "read_file", isError: false, durationMs: 300, output: "x"},
    ];
    ...
    const events2: ServerMessage[] = [
      ...events,
      { type: "tool_result", time: ms(BASE, 700), id: "t2", name: "write_file", isError: false, durationMs: 495, output: "ok"},
    ];
