//! Anthropic-shaped axum SSE fixture — shared by all omega-server integration tests.
//!
//! Mirrors `omega-cli/tests/common/mod.rs` exactly (kept in sync by hand until a
//! `omega-test-fixtures` shared crate is introduced in TEST-ARCH-3).
//!
//! Per-test scripted responses on a 127.0.0.1 random port. Tests point
//! the server subprocess at it via `ANTHROPIC_BASE_URL`. Each POST
//! `/v1/messages` pops one [`MockResponse`] from the script in order.
...
pub struct MockServer {
    pub base_url: String,
    handle: JoinHandle<()>,
    /// Remaining scripted responses. Useful for verifying the script was
    /// fully consumed.
    pub script: Script,
}
