...
- `src/diagnosis.ts` — `writeDiagnostic(data)` writes a JSON snapshot to `diagnosis/<ISO-timestamp>.json` at repo root containing verbatim error, HTTP status, exact `requestMessages`, `systemBlocks`, full `history`, model, provider, call number, and `_instructions`; `checkDiagnostics()` returns existing snapshot paths sorted oldest-first; all I/O errors silently swallowed; `diagnosis/` is in `.gitignore`
...
`diagnosis/` at repo root (gitignored) holds fatal API error snapshots written by `src/diagnosis.ts`. At startup, `app.ts` warns if any exist. After every turn footer, `app.ts` checks again and prints a red reminder line if any exist — ensuring the operator cannot miss them even if the original error scrolled away. If `diagnosis/` contains files, read them before doing anything else.
...
Design phase only. Core insight: each turn is a sequence of context states `C₀ → C₁ → … → Cₙ`; a queued prompt transforms a future state. Key unresolved questions: injection granularity, storage/delivery mechanism, whether queued prompts inject into current turn or start a new turn, UI affordance. Two anchor use cases: (UX-Q1) Esc semantics for hard stop, (UX-Q2) redirecting a turn mid-flight. No code changes yet.

### Web Interface
*See `plan/future.md` — `[TOPIC] Web interface` for WEB-1 through WEB-5.*

Direction decided: Bun HTTP server + WebSocket for `AgentEvent` streaming, Solid.js frontend, SSH tunnel for remote access, `Turn[]` store model. Next concrete task: WEB-1 — `src/web/server.ts` (Bun HTTP server + WebSocket, serving `Ag