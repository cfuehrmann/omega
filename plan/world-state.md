...
### Recent Session
Fixed two bugs: `status` events (including `/help` output and slash-command confirmations) were silently discarded by a no-op handler in `src/ui-raw.ts`; replaced with `printBlock(now(), event.message.split("\n"))` so all status messages now render correctly. All 282 tests pass.
