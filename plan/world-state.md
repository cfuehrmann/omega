...
- `src/ui-raw.test.ts` — 358 tests for raw terminal key handling and line editing; includes bracketed paste correctness tests (non-empty buffer, mid-buffer, wrap-safe path, multi-row span) and a latency guard asserting exactly 1 `stdout.write` per char-append on a 5,000-char buffer
...
### Most Recent Session
Fixed two bugs in `src/ui-raw.ts`: bracketed paste `[201~` was re-echoing pre-paste buffer content from the wrong cursor position (fixed by recording `startVisualCol`/`startCursor` at `[200~` and redrawing correctly at `[201~`); and every printable-character keypress was O(n) in buffer length due to a full string spread (fixed with an O(1) BMP-append fast path for end-of-buffer inserts). Six new tests were added to `src/ui-raw.test.ts`; all 358 tests pass.