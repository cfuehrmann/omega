...
### Most Recent Session
No code was changed. Added `[TOPIC] Prompt queuing` as the top-priority item in `plan/future.md` with full framing: the context-sequence mental model, five user intents (soft interrupt, hard stop, append, inject, replace), two design-anchor use cases (UX-Q1/UX-Q2), open design questions, and five implementation sub-tasks gated on a design decision. Updated `plan/world-state.md` to reference the new section.