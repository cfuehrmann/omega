...
- `src/logger.ts` — pino-backed structured logger. On module load rotates `omega.log → omega.prev.log` (exactly 2 sessions retained). Writes JSON-lines synchronously (no buffering — log volume is far too low for async buffering to matter, and sync is simpler/more reliable). `OMEGA_LOG_FILE` env var overrides the log path and skips rotation — used by `src/test-setup.ts` preload to redirect test output to `/dev/null`, preventing test runs from touching `omega.log`. `flushLog()` is a no-op shim (kept so call sites still compile; cleanup deferred). `startup()` convenience wrapper. `makeLogEntry()` factory for taxonomy-compliant `MessageEntry`/`InfraEntry` discriminated union shapes. `omega.log` and `omega.prev.log` in `.gitignore`.
...
- `plan/future.md` — backlog; `[TOPIC] Prompt queuing` section at top (highest priority); `[TOPIC] Provider feature parity & architecture` with ARCH-1, FEAT-2–4; FEAT-3 marked high priority; `[INFRA] Self-protection` with REC-1–4; `[TOPIC] Web interface` WEB-1–6 all done; TOOLS-1–3 done, TOOLS-4 open
...
### Recent Sessions
- Web UI status bar (`StatusDot` / `Ω Omega` header) moved from top of layout to sticky position between feed and input area; CSS borders/padding updated.
- **OAuth 403 "token revoked" fix**: `isAuthExpired()` widened to match HTTP 403 with "revoked" in message body (previously only matched 401). The existing `reinitAuth()` recovery path already handled re-auth correctly — it just never triggered. 2 new tests. Commit `4b3688a`.
- **Test log isolation**: `src/test-setup.ts` Bun preload sets `OMEGA_LOG_FILE=/dev/null` before any test module import, preventing test runs from touching `omega.log`. `bunfig.toml` preloads it. `src/logger.ts` skips rotation when `OMEGA_LOG_FILE` is set. `getLogFile()` exported. 1 new test. Commit `19f75ad`.
- **Pino sync write simplification**: removed async buffering (`minLength`/`periodicFlush`); switched to `sync: true`. `flushLog()` is now a no-op shim. No behaviour change at real log volumes. Commit `c2473d0`.
...
- REC-1 through REC-4: self-protection backlog items open (see future.md).