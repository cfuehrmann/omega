...
- `plan/future.md` — backlog; `[TOPIC] Prompt queuing` section at top (highest priority) with full framing; `[TOPIC] Provider feature parity & architecture` section with ARCH-1, FEAT-2–4; FEAT-3 marked high priority — do soon; `[TOPIC] Web interface` with WEB-1 through WEB-5 sub-tasks; TOOLS-1–3 done, TOOLS-4 open
...
### Test Files
- `src/agent-integration.test.ts` — full sendMessage loop tests including slash command tests; Anthropic `/help` test asserts presence of `1×`, `1.25×`, `0.1×` multipliers; test asserting parallel dispatch (both `tool_call` events before any `tool_result`); latch-based regression test for compaction race (verifies no orphaned `tool_result` blocks after overlapping compaction and next-turn tool use)
...
- `src/ui-raw.test.ts` — 363 tests for raw terminal key handling and line editing (imports from `src/terminal/input.ts` via the `ui-raw.ts` shim); includes bracketed paste correctness tests, latency guard, and tests for plain Delete key (forward-delete char under cursor, no-op at end of line)
- `src/entry.test.ts` — structural invariant tests; includes two tests asserting that `src/terminal/input.ts`, `renderer.ts`, `app.ts` exist and that `src/ui-raw.ts` re-exports `parseKeys`, `displayWidth`, `renderToolStart`, `renderToolResult` (guards against accidental deletion of the shim or new terminal modules)
...
### Web Interface
*See `plan/future.md` — `[TOPIC] Web interface` for WEB-1 through WEB-5 sub-tasks.*

The terminal split (`src/terminal/`) established the seam. Direction decided: Bun HTTP server + WebSocket for `AgentEvent` streaming, Solid.js frontend, SSH tunnel for remote access, `Turn[]` store model. `renderer.ts` is the model for a future web renderer emitting JSON/SSE instead of ANSI; `app.ts` will fork into `terminal/app.ts` vs `web/server.ts` when the WebSocket layer is added. Next concrete task: WEB-1 — `src/web/server.ts` (Bun HTTP server + WebSocket, serving `AgentEvent`s as JSON, accepting user messages).

### Most Recent Session
Fixed a compaction race condition where `compactAfterTurn