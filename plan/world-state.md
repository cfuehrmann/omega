...
- **Step 3** (DONE through 3e-iii): Replace `MessageParam[]` history with an event-list data structure.
  ...
  - **[SCHEMA] pre-lock fixes** (DONE, commits b59ba48, aefffb6): `LlmResponseEvent.content` removed (duplication — authoritative record is `context.jsonl`); `LlmCallEvent.messageCount` removed (redundant with `contextHashes.length`); `LlmCallEvent.llmCallNumber` removed (derivable counter — session index = count `llm_call` events; turn index = count since last `user_message`). All were breaking changes that had to land before the schema lock. `LlmResponseEvent.usage` records all four Anthropic token counts (`input_tokens`, `output_tokens`, `cache_creation_input_tokens?`, `cache_read_input_tokens?`).
...
Completed **pre-schema-lock field removals** (commits b59ba48, aefffb6): removed `LlmResponseEvent.content` (duplicated `context.jsonl`), `LlmCallEvent.messageCount` (always equalled `contextHashes.length`), and `LlmCallEvent.llmCallNumber` (derivable counter). Integration tests assert all three fields are absent at runtime. `LlmResponseEvent.usage` already records all four Anthropic token counts (`input_tokens`, `output_tokens`, `cache_creation_input_tokens?`, `cache_read_input_tokens?`) — confirmed by inspecting live `events.jsonl`. Schema lock sub-steps (3e-iv through 3e-viii) are now unblocked.

Completed **tool_call/tool_result contextHash FK + field cleanup** (commit 34f7708): `ToolCallEvent` now carries `contextHash: string` (hash of the assistant `context.jsonl` record containing the `tool_use` block); `input` field removed (content is in `context.jsonl`). `ToolResultEvent` now carries `contextHash: string` (hash of the user `context.jsonl` record containing the `tool_result` block(s)); `outputLength` removed (derivable). In `agent.ts`, the assistant hash is captured from `appendToHistory()` and passed to `tool_call` logEvents; tool_result logEvents moved to after `appendToHistory()` for the user message so the hash is available. Four new `[SCHEMA]` tests in `context-hash.test.ts` verify correctness. All FK pointers across the schema now use the hash-based pattern consistently.
