...
- `src/terminal/app.ts` — `runApp`, `shutdown`, `setupRawInput`, `printPrompt`, and the full agent-event loop. Calls `initLogger()` as its very first statement. Shutdown drains `foldCurrentSessionIntoWorldState()`. Bracketed paste mode enabled at startup. Prints diagnostic warnings at startup and after each turn footer.
