...
- `plan/future.md` — backlog; `[TOPIC] Provider feature parity & architecture` section with five prioritised items (ARCH-1 through FEAT-4)
...
- **FEAT-1** — Parallel tool execution: **done**. `Promise.all` over tool use blocks; `tool_call`/`tool_result` events carry `id`; UI shows short ID suffix for matching.
...
- `src/tool-renderers.test.ts` — tests for `renderToolStart` and `renderToolResult`; all calls now pass `id` as third/second argument; tests assert short ID suffix appears in rendered output
- `src/ui-raw.test.ts` — 231+ tests for raw terminal key handling and line editing

### Most Recent Session
Added a `[TOPIC] Provider feature parity & architecture` section to `plan/future.md` with five prioritised items (ARCH-1, FEAT-1–4) and a matching summary section in `plan/world-state.md`. Implemented FEAT-1 (parallel tool execution via `Promise.all`) with a confirming test. Added tool call ID display to the UI: `renderToolStart` and `renderToolResult` now both require an `id` parameter and render the last 6 chars as a dim bracketed suffix (e.g. `[XYZ789]`), enabling visual matching of concurrent tool calls in the terminal. 312 tests pass.