...
- `src/turn-footer.ts` — `formatTurnFooter(turn, session, provider, model)` returns `{ turnLine, sessionLine }` — two ANSI-dimmed labelled lines; appends `cache_write: N` and/or `cache_read: N` to turn/session lines when non-zero; `TurnMetrics` and `SessionTotals` include optional `cacheCreationTokens?`/`cacheReadTokens?`
...
- `src/turn-footer.test.ts` — 17 tests for `formatTurnFooter` including 6 for cache fields
...
### Web Search
`BRAVE_SEARCH_API_KEY` is set in `.env` (gitignored; Bun loads automatically). Brave Search is the primary engine; DuckDuckGo is the no-key fallback. The system prompt instructs the LLM to use `web_search` freely rather than guessing or relying on stale training data.

### AgentEvent Union (notable members)
- `world_state_saved` — `{ type: "world_state_saved"; path: string; charCount: number }` — emitted by `foldCurrentSessionIntoWorldState()` after writing the world file; rendered as