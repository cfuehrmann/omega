...
impl Agent {
    ...
    pub async fn run(&mut self, inbox: InputQueue, cancel: CancellationToken) {
        ...
        loop {
            ...
            self.drive_turn(items, inbox.clone(), cancel.clone()).await;
    ...
    async fn drive_turn(
        ...
        if !dangling.is_empty() {
            ...
            match self.inject_dangling_tool_results(dangling).await {
                Ok(events) => {
                    for ev in events {
                        self.event_sink.push_to_wire(AgentItem::event(ev));
                    }
                }
                Err(e) => {
        ...
        for item in items {
            match self.inject_input_item(item).await {
                Ok(ev) => self.event_sink.push_to_wire(AgentItem::event(ev)),
                Err(e) => {
                    let ev = OmegaEvent::AgentError(AgentErrorEvent {
                        time: now_iso(),
                        error: format!("context_store append failed: {e}"),
        ...
        loop {
            ...
            let request = LlmRequest {
                ...
                config: ModelConfig {
                    ...
                    effort: Some(cap_effort_for_model(&active_effort, &active_model).to_owned()),
                },
            ...
            self.commit_event(call_ev).await;
            ...
            while let Some(item) = provider_stream.next().await {
                ...
                if !response_started {
                    ...
                    let started =
                        OmegaEvent::LlmResponseStarted(LlmResponseStartedEvent { time: now_iso() });
                    self.commit_event(started).await;
                }
                    ...
                    Ok(AgentItem::Signal(sig)) => {
                        ...
                        let forward = match &sig {
                            ...
                            StreamSignal::ThinkingBlockComplete { index, signature } => {
                                ...
                                block_event = Some(OmegaEvent::ThinkingBlock(ThinkingBlockEvent {
                                    time: now_iso(),
                            ...
                            StreamSignal::TextBlockComplete { index, text } => {
                                ...
                                block_event = Some(OmegaEvent::TextBlock(TextBlockEvent {
                                    time: now_iso(),
                                    text: text.clone(),
                                    partial: false,
                                }));
                            ...
                            StreamSignal::ToolUseBlockComplete {
                                ...
                                block_event = Some(OmegaEvent::ToolUseBlock(ToolUseBlockEvent {
                                    time: now_iso(),
                                    tool_call_id,
                        ...
                        if let Some(be) = block_event {
                            self.commit_event(be).await;
                        }
                        if forward {
                            self.event_sink.emit_signal(sig);
                        }
                    }
                        ...
                        match event {
                            ...
                            OmegaEvent::LlmResponseEnded(lr) => {
                                    ...
                                    && iters.iter().any(|it| it.iteration_type == "compaction")
                                {
                                    ...
                                    compacted_info = Some(extract_compaction_tokens(iters));
                                }
                            ...
                            OmegaEvent::LlmRetry(retry) => {
                                ...
                                if response_started {
                                    for closer in
                                        make_abandonment_closers(std::mem::take(&mut slots))
                                    {
                                        self.commit_event(closer).await;
                                    }
                                }
            ...
            if let Some(err) = stream_error {
                ...
                self.commit_event(llm_err_ev).await;

                if is_invalid_tool_json(&err) && feedback_attempts < INVALID_TOOL_JSON_FEEDBACK_CAP
                {
                    feedback_attempts += 1;
                    match self
            ...
            for (id, name, input) in &tool_uses {
                combined_tool_uses.push((id.clone(), id.clone(), name.clone(), input.clone()));
                assistant_blocks.push(ContentBlock::ToolUse {
                    id: id.clone(),
                    name: name.clone(),
                    input: input.clone(),
            ...
            if assistant_blocks.is_empty() {
                ...
                if let Some((tokens_before, tokens_after, summary_tokens)) = compacted_info.take() {
                    ...
                    self.commit_event(cc_ev).await;
                }
                let ended_ev = OmegaEvent::LlmResponseEnded(lr);
                self.commit_event(ended_ev).await;
            ...
            let assistant_hash = match self.append_record(Role::Assistant, assistant_blocks).await {
                Ok(h) => h,
                Err(e) => {
                    let ev = OmegaEvent::AgentError(AgentErrorEvent {
                        time: now_iso(),
            ...
            if stop_reason == "tool_use" && !combined_tool_uses.is_empty() {
                ...
                // error.
                let python_repl_opt = if self.tool_selection.iter().any(|n| n == "python_repl") {
                ...
                while let Some((_idx, tool_call_id, name, res, elapsed)) = futures.next().await {
                    ...
                    for ev in &res.extra_events {
                        self.commit_event(ev.clone()).await;
                    }
                ...
                let result_blocks: Vec<ContentBlock> = combined_tool_uses
                    ...
                    .map(|(tool_call_id, tool_use_id, _, _)| {
                        ...
                        let (content, is_error) =
                            by_call_id.remove(tool_call_id).unwrap_or_else(|| {
                ...
                if self.controls.take_halt_request() {
                    let halted_ev = OmegaEvent::TurnHalted(TurnHaltedEvent { time: now_iso() });
                    self.commit_event(halted_ev).await;
                    ...
                    let resumed_item: Option<InputItem> = if self.controls.take_resume_request() {
                        None
                    } else {
                        tokio::select! {
                    ...
                    let resumed_ev = OmegaEvent::TurnResumed(TurnResumedEvent { time: now_iso() });
                    self.commit_event(resumed_ev).await;
                }
