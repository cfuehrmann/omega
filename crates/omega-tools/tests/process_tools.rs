...
//! Integration tests for `run_command` and the shared tee / output-cleaner
//! behaviour.  The background-job tools (`run_background` / `write_stdin`,
//! which now run *through* the `MonitorManager`) and `stop_monitor` are tested
//! end-to-end in `src/monitors.rs`, where the manager test helpers live.
